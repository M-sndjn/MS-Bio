  ...
  */}
  ...
  return (
    ...
    <section ref={sectionRef} className="relative min-h-[100dvh] mt-10 ml-10">
       ...
       */}
      <div className="top-30 h-[100dvh] flex flex-col space-y-36 sm:space-y-40">
        
        <div className="mix-blend-difference space-y-10">
          <div>
            <h2 className="text-3xl sm:text-4xl sm:px-10 py-5 px-2">Skills</h2>
            <span className="text-white sm:px-10 px-2 block text-md sm:text-lg w-[54%]">What I actively use, what I’ve applied in projects, and what I’m currently exploring.</span>
          </div>
        ...
        <div className=" h-30 w-full mix-blend-difference">
          {level && level !== "Outro" && (
            <span
...
function Skill({ children, className = "" }) {
  return (
    <div className="w-10 h-10  sm:w-18 sm:h-18 flex items-center justify-center">
      <div className={`w-7 h-7 sm:w-12 sm:h-12 text-white ${className}`}>
        {children}
      </div>
    </div>
  );
